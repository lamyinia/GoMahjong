...
func NewGameService(roomManager *game.RoomManager, worker *game.Worker) GameService {
...
// CreateRoom 创建游戏房间
func (s *GameServiceImpl) CreateRoom(ctx context.Context, req *CreateRoomReq) (*CreateRoomResp, error) {
	if req == nil {
	...
	// 推送匹配成功消息给所有玩家
	matchSuccessMsg := map[string]interface{}{
		"roomID":  room.ID,
		"players": req.Players,
	}
	...
	for userID := range req.Players {
		if err := s.worker.PushMessage(userID, "game.matchSuccess", msgData); err != nil {
