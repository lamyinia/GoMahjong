...
const TILE_CHARS = {
  // Wan (Characters) - 1-9
  wan: ['\u{1F019}', '\u{1F01A}', '\u{1F01B}', '\u{1F01C}', '\u{1F01D}', 
        '\u{1F01E}', '\u{1F01F}', '\u{1F020}', '\u{1F021}'],
  // Tiao (Bamboo) - 1-9
  tiao: ['\u{1F010}', '\u{1F011}', '\u{1F012}', '\u{1F013}', '\u{1F014}',
         '\u{1F015}', '\u{1F016}', '\u{1F017}', '\u{1F018}'],
  // Tong (Dots) - 1-9
  tong: ['\u{1F007}', '\u{1F008}', '\u{1F009}', '\u{1F00A}', '\u{1F00B}',
...
const TILE_TYPES = ['wan', 'tiao', 'tong', 'feng', 'dragon'] as const;

export function tileToChar(tile: Tile): string {
  const typeKey = TILE_TYPES[tile.type];
  const chars = TILE_CHARS[typeKey];
  return chars[tile.id - 1] || '?';
}

export function createTileElement(tile: Tile, clickable = false): HTMLElement {
  const el = document.createElement('span');
  el.className = `tile ${TILE_TYPES[tile.type]}`;
...
export function tileToName(tile: Tile): string {
  const typeNames = ['Wan', 'Tiao', 'Tong', 'Feng', 'Dragon'];
  ...
  const typeKey = TILE_TYPES[tile.type];
  const names = idNames[typeKey];
  return `${names[tile.id - 1]} ${typeNames[tile.type]}`;
}
